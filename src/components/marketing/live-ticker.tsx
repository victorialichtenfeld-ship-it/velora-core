...
export function LiveTicker() {
  ...
  return (
    <div className="group/ticker relative overflow-hidden border-y border-border bg-card py-3">
      ...
      <p className="absolute top-1/2 left-3 z-20 -translate-y-1/2 text-[10px] font-medium uppercase tracking-[0.14em] text-primary">
        Sample
      </p>
      <div className="animate-ticker flex w-max gap-10 whitespace-nowrap px-6 pl-24 text-[12px] text-muted-foreground group-hover/ticker:[animation-play-state:paused]">
        {loop.map((item, index) => (
          <span
            key={`${item}-${index}`}

...
export function LiveTicker() {
  ...
  return (
    <div className="relative overflow-hidden border-y border-gold/25 bg-gold/[0.08]">
      ...
      <div className="animate-ticker flex w-max gap-16 whitespace-nowrap py-5 text-[13px] text-muted-foreground">
        {loop.map((item, index) => (
          <span key={`${item.event}-${index}`} className="flex items-center gap-4" aria-hidden={index >= items.length}>
            <span className="rounded-full bg-protect/15 px-2 py-0.5 text-[11px] font-medium text-protect">Held</span>

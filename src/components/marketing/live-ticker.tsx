...
const amounts = ["$11,240 held", "$14,760 held", "$8,400 held", "$6,480 held", "$9,120 held", "$4,275 held", "$184,320 vault"];

export function LiveTicker() {
  const loop = [...items, ...items];
  const cash = [...amounts, ...amounts];
  return (
    <div className="group/ticker relative overflow-hidden border-y border-gold/25 bg-card">
      ...
      <div className="animate-ticker flex w-max gap-10 whitespace-nowrap py-2.5 pr-6 pl-24 text-[12px] text-muted-foreground group-hover/ticker:[animation-play-state:paused]">
        {loop.map((item, index) => (
          <span
          ...
          >
            <span className="size-1.5 rounded-full bg-gold shadow-[0_0_8px_rgb(201_168_106_/_0.8)]" />
            <span className="font-figure text-gold">{item.time}</span>
            <span>{item.event}</span>
            <span className="text-foreground/85">{item.party}</span>
            <span className="font-figure text-gold">{item.amount}</span>
          </span>
        ))}
      </div>
      <div className="animate-ticker-reverse flex w-max gap-8 border-t border-bronze/20 py-1.5 pr-6 pl-24 text-[12px] text-gold group-hover/ticker:[animation-play-state:paused]">
        {cash.map((amount, index) => (
          <span key={`${amount}-${index}`} aria-hidden={index >= amounts.length} className="font-figure tracking-[0.04em]">
            {amount}
          </span>
        ))}
      </div>

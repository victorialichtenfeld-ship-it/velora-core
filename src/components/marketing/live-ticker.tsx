...
export function LiveTicker() {
  return (
    <div className="relative overflow-hidden py-2">
      ...
      <Row />
      <Row reverse />
      <div className="animate-ticker-fast flex w-max gap-4 whitespace-nowrap py-3 text-[13px] text-muted-foreground">
        {[...items, ...items].map((item, index) => (
          <span key={`c-${item.event}-${index}`} className="glass inline-flex items-center gap-3 rounded-full px-3 py-1.5" aria-hidden={index >= items.length}>
